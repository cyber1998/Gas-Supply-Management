...
def success():
	...
	if request.method == "POST":
		...
		connectionid = aadhar+str(datetime.now().second)+str(datetime.now().year)
		print(connectionid)
		...
		try:
			customerMainInsert(aadhar, user, password, email, name, address, city, connectionid)
...
def changepassword():
	...
	if request.method == 'POST':
		...
		cursor = db.cursor()
		cursor.execute('''SELECT password FROM MasterTable WHERE username = %s''', (user,))
		tup = cursor.fetchone()
		print(tup[0])
		if oldpassword == tup[0]:
			if newpassword == confirm_new:
				try:
					cursor.execute('''UPDATE MasterTable SET password = %s WHERE username = %s''', (newpassword, user))
					print("MasterTable updated")
					cursor.execute('''UPDATE Users SET password = %s WHERE username = %s''', (newpassword, user))
					print("Users updated")
					cursor.execute('''UPDATE CustomerDetails SET password = %s WHERE username = %s''', (newpassword, user))
...
def changeaddress():
	...
	if request.method == 'POST':
		...
		try:
			...
			cursor.execute('''UPDATE MasterTable SET address = %s WHERE username = %s''', (newaddress, user))
...
def userpage(user):
	...
	if g.username:
		db = MySQLdb.connect(host = "127.0.0.1", user =  "customer", passwd = "password", db = "FlaskDB",port=5000)
		cursor = db.cursor()
		cursor.execute('''SELECT name, address, city, connection_id FROM MasterTable WHERE username = %s''', (user,))
		tup = cursor.fetchone()
...
def auth():
	if request.method == 'POST':
		...
		db = MySQLdb.connect(host = "127.0.0.1", user =  "customer", passwd = "password", db = "FlaskDB",port=5000)
		cursor = db.cursor()
		cursor.execute('''SELECT username, password FROM Users where username = %s''', (signinuser,))
		...
		# if signinuser == "" or signinuser not in result:
		# 	return render_template("signin.html", error = "Invalid Username")
		# if signinpass == "" or signinpass not in result:
...
def send_complain():
	...
	if request.method == 'POST':
		...
		complaint_id = signinuser[0:5]+str(datetime.now().hour)+str(datetime.now().minute)+str(datetime.now().second)
		complain = request.form['complainbox']
		print(complaint_id)
		try:
			db = MySQLdb.connect(host = "127.0.0.1", user =  "customer", passwd = "password", db = "FlaskDB",port=5000)
			cursor = db.cursor()
			cursor.execute('''	INSERT INTO UserComplaints (username, complaint_id, complaint) values (%s,%s,%s)''',(signinuser,complaint_id,complain,))
			db.commit()
	...
	if request.method == 'POST':
		if request.form['forfeit'] == 'Yes':
			...
			try:
				db = MySQLdb.connect(host = "127.0.0.1", user =  "customer", passwd = "password", db = "FlaskDB",port=5000)
				cursor = db.cursor()
				cursor.execute('''DELETE FROM MasterTable where username = %s''', (signinuser,))
				cursor.execute('''DELETE FROM Users where username = %s''', (signinuser,))
				cursor.execute('''DELETE FROM CustomerDetails where username = %s''', (signinuser,))
				db.commit()
			except:
